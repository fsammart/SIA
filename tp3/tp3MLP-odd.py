...
def multi_layer_perceptron(eta, iterations):
    ...
    y = np.reshape(b,(X.shape[0],2))

    net = mlp.Mlp([digit_map_height*digit_map_width,10,5,10,2], momentum=0.5, eta=0.001, eta_adaptative=True, iter_update_eta=5,
                  eta_increment=0.001, eta_decrement=0.1, precision=0.01, min_eta=0.00001, params=0.5)


    y

    net.train(X[:5,:], y[:5], iterations=20000, reset=False, batch=True)
    #print(R[i])
